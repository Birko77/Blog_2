    ...
    def get(self):
        if self.user:
            ...
            if self.user.key().id() == article.author:
                ...
                self.session.add_flash('message_edit_article_2', 
                                       key='homepage_flashes')
                self.redirect("/")
        else:
            # Prompt user to login.
            self.session.add_flash('message_edit_article_1', 
                                   key='homepage_flashes')
            self.redirect("/")
    ...
    def post(self):
        if self.user:
            ...
            if input_delete_article:
                ...
                self.session.add_flash('message_delete_article', 
                                       key='homepage_flashes')
                self.redirect("/")
...
class SendEmailHandler(Handler):
    ...
    def post(self):
        if self.user:
            ...
            # Prompt user to login.
            self.session.add_flash('message_send_email_1', 
                                   key='homepage_flashes')
            self.redirect("/")
